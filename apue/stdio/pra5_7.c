...
static int type_to_flags(const char *restrict type);
...
static int stream_write(void *cookie, char *buf, int len)
{
	int nw, off;
	struct memstream *ms = cookie;

	if ((ms->flags & (MS_WRITE | MS_APPEND)) == 0) {
		errno = EBADF;
		return (-1);
	}
