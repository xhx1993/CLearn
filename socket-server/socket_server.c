...
struct socket_server {
	int recvctrl_fd;	//接受控制消息的socket
	int sendctrl_fd;	//发送控制消息的socket
	int checkctrl;		//是否检查控制消息标记
	poll_fd event_fd;	//epoll的efid
	int alloc_id;		//当前分配的socket数量（递增)
	int event_n;		//当前可处理epoll事件数量
	int event_index;	//当前处理的epoll事件id
...
socket_server_poll(struct socket_server *ss, struct socket_message * result, int * more) {
	for (;;) {
		...
		case SOCKET_TYPE_CONNECTING:
			//对一个非阻塞socket, con时未完成。如果对方处理完成，返回可写
